...
private HashMap<Integer,String> labelRef = new HashMap<Integer,String>(); // Contiene i riferimenti ai buchi lasciati vuoti.
// Non gestiamo l'errore nel caso in cui salto ad un etichetta che non viene mai generata.
}
...
assembly: instruction* EOF {
	// Riempio i vari buchi con la loro definizione.
	for(Integer j: labelRef.keySet()) { // Per ogni riferimento mancante
		code[j] = labelDef.get(labelRef.get(j)); // Inserisco l'integer corrispondente
	}
};

instruction:
        PUSH n = INTEGER { // Abbiamo dichiarato una variabile relativa a INTEGER
        	code[i++] = PUSH;
        	code[i++] = Integer.parseInt($n.text); // Ottengo il lessema che ha generato il token
